        ...
        while retry_time < 100:
            ...
            try:
                response = requests.post(u, headers={
                    "User-Agent": agent,
                    "Referer": "https://www.virustotal.com/"}, proxies=proxies)
        ...
        while retry_time < 5:
            ...
            try:
                response = requests.get(get_url, headers={
                    ...
                    "Referer": "https://www.virustotal.com/"}, proxies=proxies)
                print("获取报告成功")
                return {"malicious": response.json()["data"]["attributes"]["stats"]['malicious'],
                        "suspicious": response.json()["data"]["attributes"]["stats"]['suspicious']}
            except:
                print("获取报告失败，重试中...")
                retry_time += 1
