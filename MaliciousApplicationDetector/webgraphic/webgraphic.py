...
class webgraphic:
    def __init__(self):
        ...
        self.recent_three = []
        self.recent_index = 0
        file = open(os.path.join(self.path, "top-10k.txt"), 'r')
        line = file.readline()
        while line:
            ...
            if re.match(self.ptr, s):
                ...
                if not ref:
                    ...
                    for k in range(len(self.tmp_mem) - 1):
                        ...
                        if self.IsSimilar(domain_tmp, host) and not self.groups[self.tmp_index[index]].is_alone() and \
                                        self.groups[self.tmp_index[index]].is_PC() == is_PC:
                            # print("使用规则4连接")
                            # print(domain_tmp)
                            # print(host)
                    ...
                    if not fflag:
                        ...
                        for k in range(len(self.tmp_mem) - 1):
                            ...
                            try:
                                ...
                                continue
                            index = len(self.tmp_mem) - 2 - k
                            if (self.IsSimilar(domain_tmp, ref_domain) or self.IsSimilar(
                                    self.groups[self.tmp_index[index]].ref_domain, ref_domain)) and not self.groups[
                                self.tmp_index[index]].is_alone() and self.groups[
                                self.tmp_index[index]].is_PC() == is_PC:
                                # print("使用规则2连接")
                                # print(index)
                                # print(s_tmp)
                                # print(s)
        ...
        for x in self.groups:
            for i in range(len(x.id)):
                ...
                if url in labled_url:
                    lable = 1
                tmp = [content_type, response_lenth, number_of_ref, time_gap, url_lenth, advertisement,
                       presence_of_parents, lable]
                features.append(tmp)
        return features
    ...
    def IsSimilar(self, domain1, domain2):
        ...
        stop_words = ["www", "com", "cn", "bbs", "edu", "gov", "int", "mil", "net", "org", "biz", "info", "pro", "name",
                      "museum", "coop", "aero"]
