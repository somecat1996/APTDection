    ...
    def classify(self,ips):
        ...
        for x in files:
            ...
            if ip in ips[-1]:
                ...
                continue
            if ip in ips[0]:
                self.browser_PC.append({"filename":x,"type":1,"is_malicious":0})
            elif ip in ips[1]:
                self.backgroud_PC.append({"filename":x,"type":2,"is_malicious":0})
            elif ip in ips[2]:
                self.browser_Phone.append({"filename":x,"type":3,"is_malicious":0})
            elif ip in ips[3]:
                self.backgroud_Phone.append({"filename":x,"type":4,"is_malicious":0})
            elif ip in ips[4]:
                self.suspicious.append({"filename":x,"type":5,"is_malicious":0})
            else:
                print("找不到",ip)
                count_no+=1
