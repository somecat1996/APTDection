    ...
    for kind, group in labels.items():
        ...
        for files in group.values():
            for file in files:
                ...
                loads(f'./newDataSet/{file}', f'./cmp/httpheader/{kind}/{label}/{dataset}')
...
def loads(fin, fout):
    ...
    print(f'\nExtracting file {fin} & dumping to {fout}')
    extractor = jspcap.extract(fin=fin, store=False, tcp=True, verbose=True, nofile=True, strict=True, extension=False, auto=True)
...
#     main()
print('Dataset ready at ./cmp/httpheader')
